...
### Fonts
All on-screen text uses the macOS system font (Helvetica Neue), set app-wide via `LookAndFeel::setDefaultSansSerifTypefaceName("Helvetica Neue")` in `Main.cpp`. No custom typeface is embedded; sites that need explicit sizing/weight construct `juce::Font(juce::FontOptions().withName("Helvetica Neue").withHeight(N).withStyle("Bold"))` directly.
